...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[280px] bg-[#141a24] text-white flex flex-col shadow-[10px_0_30px_rgba(0,0,0,0.15)] z-20 shrink-0 h-screen sticky top-0 relative">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.label} 
              to={item.path}
              className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl font-medium transition-all group ${
                isActive 
